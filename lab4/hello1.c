...
int print_hello(uint amount)
{
	...
	if (amount < 0) {
		pr_err("FAIL. Parameter < 0\n");
	} else if (amount == 0) {
		pr_warn("Warning. Parameter = 0\n\n");
	} else if (amount >= 5 && amount <= 10) {
		pr_warn("Warning. 5 > Parameter > 10\n");
	} else if (amount > 10) {
		pr_err("FAIL. Parameter > 10\n");
		return -EINVAL;
	}
...
{
	pr_info("SUCCESS. hello1 has been deployed\n");
	return 0;
}
...
static void __exit exit_hello(void)
{
	...
	pr_info("SUCCESS. hello1 has been undeployed\n");
}
